...
import { getDeploymentScenarios, createDeploymentScenario, deleteDeploymentScenario, updateDeploymentScenario } from './service';
import { useState } from 'react';
import { history } from 'umi';
import { BugOutlined, DownloadOutlined, FireOutlined, MonitorOutlined, PlusOutlined, SafetyCertificateOutlined, SecurityScanOutlined, UploadOutlined } from '@ant-design/icons';
...
import { getAssets } from '../assets/list/service';
import { getSystemProfiles } from '../system-profiles/list/service';
import { DEPLOYMENT_SCENARIO_DEPLOYMENTS, DEPLOYMENT_SCENARIO_OPERATIONS, DEPLOYMENT_SCENARIO_REQUIREMENTS_ANALYSIS, MAX_INT } from '@/shared/constant';
import * as Excel from "exceljs";
import FileSaver from 'file-saver';
import { getCountermeasures } from '../countermeasures/list/service';
import ProTable from '@ant-design/pro-table';
import IconAction, { DeleteIconAction, DetailIconAction, EditIconAction } from '@/components/TableAction';
...
const DeploymentScenarios = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            dataIndex: 'system_profile',
            key: 'system_profile',
            width: '20%',
            render: (system_profile) => system_profile[0].name,
        },
        {
            title: 'Stage',
            dataIndex: 'status',
    ...
    const exportExcel = async (systemProfile, assets, countermeasures) => {
        ...
        securityGoalWorksheet.getRow(3).values = ['ID', 'Name', 'Description', 'Asset ID', 'Confidentiality', 'Integrity', 'Availability']
        ...
        assetRelationshipWorksheet.getRow(3).values = ['ID', 'Asset ID source', 'Asset ID target', 'Access vector', 'Privilege']
        ...
        countermeasureWorksheet.getRow(3).values = ['ID', 'Name', 'Cost', 'Coverage', 'Coverage CVE_ID']
        ...
        dataAssetWorksheet.columns = [
            { key: 'id' },
            { key: 'name' },
            { key: 'part' },
        ]
        assets.forEach((asset) => dataAssetWorksheet.addRow({
            ...asset,
        ...
        dataCountermeasureWorksheet.columns = [
            { key: 'id' },
            { key: 'name' },
            { key: 'cover_cves' },
        ]
        countermeasures.forEach((countermeasure) => dataCountermeasureWorksheet.addRow({
            ...countermeasure,
    ...
    return (
        <PageContainer>
            ...
            <Modal
                ...
                destroyOnClose={true}
                wrapClassName='modal-fullscreen'
                visible={visiableCountermeasures}
