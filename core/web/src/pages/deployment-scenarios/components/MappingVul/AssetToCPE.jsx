...
import MappingCPE from "./MappingCPE";
import './index.less'
...
const AssetToCPE = ({ deploymentScenario, assets, setAssets }) => {
    ...
    const handleSaveMapping = async () => {
        ...
        const resData = await updateDeploymentScenarioOneAsset(deploymentScenario.id, currentAsset.id, sendData)
        if (resData) {
            setAssets(assets.map(as => {
                if (as.id === currentAsset.id)
                    return {
