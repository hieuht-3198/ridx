...
import { Button, Col, Form, Input, InputNumber, message, Modal, Popconfirm, Row, Space, Table, Typography } from 'antd';
...
const AttackerList = ({ deployment_scenario_id }) => {
                ...
                <ul>
                    {
                        targets.map(target => (
                            <li key={target.asset_id}>
                                {`${deploymentScenario.assets.find(as => as.id === target.asset_id).name} - ${target.attack_vector} - ${target.privilege}`}
                            </li>
                        ))
                    }
                </ul>
    ...
    const handleAddEditAttacker = async (values) => {
        ...
        let sendData = { ...values }
        const res = isAddAttacker ? await createAttacker(deployment_scenario_id, sendData) : await updateAttacker(deployment_scenario_id, currentRow.id, sendData)
        if (res) {
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            <Row style={{ marginBottom: 9 }} gutter={10}>
                                <Col span={13}>
                                    Asset
                                </Col>
                                <Col span={4}>
                                    ...
                                    <>
                                        {fields.map(({ key, name, ...restField }) => (
                                            <Space key={key} style={{ display: 'flex', marginBottom: 8 }} align="baseline">
                                                <ProFormSelect
                                                    options={
                                                        deploymentScenario?.assets?.map((asset) => ({
                                                            label: asset.name,
                                                            value: asset.id,
                                                        }))
                                                    }
