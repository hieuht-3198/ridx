    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            if (resData) {
                ...
                const tmpVal =`${resData?.cves[0]?.asset_id}_${resData?.cves[0]?.cves[0]?.cve_id}`
                setTarget(tmpVal)
                localStorage.setItem(KEY_DATA_GRAPH, JSON.stringify(resData.attack_graph))
            } else {
                message.error('Please setting all value of CVE on Asset')
            }
            setLoading(false)
        }
        fetchData()
            ...
            <GGEditor className={styles.editor}>
                ...
                <Row className={styles.editorBd}>
                    ...
                    <Col span={5} className={styles.editorSidebar}>
                        <FlowDetailPanel cves={cves} setDataGraph={setDataGraph} />
                        {/* <EditorMinimap /> */}
                    </Col>
                </Row>
                <FlowContextMenu />
