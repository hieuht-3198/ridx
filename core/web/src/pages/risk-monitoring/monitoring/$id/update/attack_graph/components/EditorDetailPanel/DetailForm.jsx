...
class DetailForm extends React.Component {
    ...
    getAssets = (asset_id) => {
        const result = this.props.cves.find((item) => item.asset_id == asset_id)
        return result ? result.asset_name : ''
    }
    ...
    renderNodeDetail = () => {
        const { label, is_attacker, cve_id, asset_id, attacker_id } = this.item.getModel();
        if (is_attacker) {
            return (
                <Form
                    initialValues={{
                ...
                <Item label={<b>CVE</b>} name="label" >
                    ...
                    >
                        {this.props.cves.map((asset) => (<>
                            <Select.OptGroup label={`${asset.asset_name}`} key={`${asset.asset_id}`}>
                                {
                                    asset.cves.map((cve) => (
                                        <Select.Option value={`${asset.asset_id}_${cve.cve_id}`} key={`${asset.asset_id}_${cve.cve_id}`}>
                                            {cve.cve_id}
                                        </Select.Option>
                                    ))
                                }
